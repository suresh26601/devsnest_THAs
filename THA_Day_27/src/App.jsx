 ...
 <input className="border-blue-700 border rounded w-3/5 px-3 py-2"
   ...
   <article className="w-full mt-4 px-4 ">
        {stateTodo.TODOS.length!==0?stateTodo.TODOS.map((ele) => {
              ...
              <button className="w-2/6 border border-red-500 rounded bg-red-400 hover:bg-red-300 text-white self-center" onClick={()=>dispatch({type:"REMOVE",payload:ele.id})}>Remove Me</button>
              </div>}):(
